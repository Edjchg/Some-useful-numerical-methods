...
def runge_kutta4(f,x_0,y_0,a,b,numeroPuntos):
...
def vectoresXY(f,h,x_0,y_0,vectorX):
    ...
    x_n = x_0 + h/2
    y_n = y_0 + h*(k1/2)
    k2 = f.evalf(subs={x:x_n,y:y_0})
    x_n = x_0 + h/2
    y_n = y_0 + (h/2)*k2
    k3 = f.evalf(subs={x:x_n,y:y_0})
    x_n = x_0 + h
    y_n = y_0 + h*k3
    k4 = f.evalf(subs={x:x_n,y:y_0})
    contador += 1
    Y[contador] = y_0 + (h/6)*(k1 + 2*k2 + 2*k3 + k4)
    while contador != m-1:
        y_n = Y[contador]
        ...
        k2 = f.evalf(subs={x:x_n,y:y_0})
        x_n = vectorX[contador] + h/2
        y_n = Y[contador] + h*(k2/2)
        k3 = f.evalf(subs={x:x_n,y:y_0})
        x_n = vectorX[contador] + h
        y_n = Y[contador] + h*k3
        k4 = f.evalf(subs={x:x_n,y:y_0})
        contador += 1
        Y[contador] = Y[contador - 1] + (h/6)*(k1 + 2*k2 + 2*k3 + k4)
    return [vectorX, Y]
...
runge_kutta4('-x*y + 4*x/y',0,1,0,1,11)
